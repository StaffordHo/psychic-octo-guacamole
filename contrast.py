...
def dpm_enhance(img):
    # Dot-peen pop/contrast: CLAHE -> median -> white/black top-hat -> slight close.
...
# ---------- NEW: contrast stage (before locating/decoding) ----------
# Trackbar-driven parameters live in the "Preproc" window.
MODES = ["None", "Linear", "CLAHE", "Gamma", "DotPeen"]

def apply_contrast(gray, mode:int, alpha:float, beta:int, clip:float, gamma:float):
    """
    gray  : uint8 ROI
    mode  : 0=None, 1=Linear, 2=CLAHE, 3=Gamma, 4=DotPeen
    alpha : gain for Linear (e.g., 1.20)
    beta  : offset for Linear (0..100 typical)
    clip  : CLAHE clip limit (e.g., 2.5)
    gamma : gamma correction (e.g., 1.40)
    """
    if mode == 0:
        return gray
    if mode == 1:  # Linear gain/offset
        out = cv2.convertScaleAbs(gray, alpha=alpha, beta=beta)
        return out
    if mode == 2:  # CLAHE
        cla = cv2.createCLAHE(clipLimit=max(0.1, clip), tileGridSize=(8,8))
    ...
    if mode == 4:  # DotPeen bundle
        return dpm_enhance(gray)
    return gray
...
def main(img_path):
    ...
    H,W = src.shape
    win = "DM Watch (drag ROI; q=quit, r=reset, a=all, d=dump)"
    cv2.namedWindow(win, cv2.WINDOW_NORMAL)

    # NEW: Preproc control panel
    cv2.namedWindow("Preproc", cv2.WINDOW_AUTOSIZE)
    cv2.createTrackbar("Mode", "Preproc", 4, 4, lambda v: None)         # default DotPeen
    cv2.createTrackbar("Alpha_x100", "Preproc", 120, 300, lambda v: None) # Linear gain (1.20)
    cv2.createTrackbar("Beta", "Preproc", 10, 100, lambda v: None)        # Linear offset
    cv2.createTrackbar("CLAHE_clip_x10", "Preproc", 25, 80, lambda v: None) # 2.5
    cv2.createTrackbar("Gamma_x100", "Preproc", 140, 300, lambda v: None)   # 1.40
    ...
    def worker():
        ...
        while not stop.is_set():
            # read preproc settings
            mode = cv2.getTrackbarPos("Mode","Preproc")
            alpha = cv2.getTrackbarPos("Alpha_x100","Preproc") / 100.0
            beta  = cv2.getTrackbarPos("Beta","Preproc")
            clip  = cv2.getTrackbarPos("CLAHE_clip_x10","Preproc") / 10.0
            gamma = cv2.getTrackbarPos("Gamma_x100","Preproc") / 100.0

            with lock:
                v = state["version"]
                x0,y0,x1,y1 = state["roi"]
                raw = src[y0:y1, x0:x1].copy()
                want_all = state["all"]

            if v != last_v or (time.time()-last_t) > 0.5:
                last_v = v; last_t = time.time()
                if raw.shape[0] >= 24 and raw.shape[1] >= 24:
                    # ---- NEW: apply contrast BEFORE locating/decoding ----
                    gray = apply_contrast(raw, mode, alpha, beta, clip, gamma)
    ...
    try:
        while True:
            with lock:
                ...
                ranked = state["ranked"]
                want_all = state["all"]
                pre = state.get("preproc_preview", None)
            ...
            # Tiny preview of the PREPROCESSED ROI so you can SEE the contrast effect
            if pre is not None and pre.size > 0:
                thumb = pre.copy()
                th = 160
                tw = int(thumb.shape[1] * (th / max(1,thumb.shape[0])))
                thumb = cv2.resize(thumb, (tw, th), interpolation=cv2.INTER_NEAREST)
                cv2.imshow("Preprocessed ROI", thumb)
            ...
            elif k == ord('d'):  # dump current raw & preproc
                with lock:
                    raw = src[y0:y1, x0:x1].copy()
                    preimg = state.get("preproc_preview", raw)
                cv2.imwrite("dump_raw.png", raw)
                cv2.imwrite("dump_preproc.png", preimg)
                print("Saved dump_raw.png and dump_preproc.png")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python dm_watch_contrast.py <image>")
        sys.exit(1)
    main(sys.argv[1])
